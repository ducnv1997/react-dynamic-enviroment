...
const isExistEnvFile = fs.existsSync(envFilePath)


function getKeyInDotEnv () {
  const dotEnvFileString = fs.readFileSync(envFilePath).toString()
  const dotEnvData = parse(dotEnvFileString);
  return Object.keys(dotEnvData)
}

const keysInDotEnvFile = isExistEnvFile ? getKeyInDotEnv() : [];
/** Load app's .env file */
...
function patchIndexHtml(html) {
  ...
  if ($("script#react-dynamic-environment").length) {
    $("script#react-dynamic-environment").attr("src", `%PUBLIC_URL%/env.js`);
  } else {
    $("head").append(`\t<script id="react-dynamic-environment" src="%PUBLIC_URL%/env.js"></script>\n\t`);
  }
